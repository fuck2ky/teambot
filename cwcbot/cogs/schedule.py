    ...
    async def schedule(self, context, *args):
        ...
        if args:
            ...
            for day in range(5, 7):
                await self.schedule_day(context, calendar.day_name[day])

    async def schedule_day(self, context, day, start=0):
        await context.send(f'`{day}`')

        times = [t for t in range(start, 24, 3)]

        for time in times:
            full_time = (
                f"EST {t_add(time, 0)} - {t_add(time, 3)}"
                f"\nUK  {t_add(time, 5)} - {t_add(time, 8)}"
                f"\nEU  {t_add(time, 6)} - {t_add(time, 9)}"
                f"\nJAP {t_add(time, 15)} - {t_add(time, 18)}"
            )
